...
export class UserController {
  ...
  static async createUser(req, res) {
    try {
      const { name, email, password, role, managerId, isManagerApprover = false } = req.body;
      ...
      // Validate role
      const validRoles = ['ADMIN', 'MANAGER', 'EMPLOYEE', 'FINANCE', 'DIRECTOR'];
      ...
      if (managerId) {
        ...
        if (!manager) {
          return res.status(400).json({ error: "Manager not found in your company" });
        }
      }
      // Hash password
  ...
  static async getManagers(req, res) {
    try {
      ...
      console.log('Available roles in database:', distinctRoles.map(r => r.role));
