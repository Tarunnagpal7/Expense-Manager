 ...
 */
 const overviewReport = async (req, res) => {
  try {
    ...
    const convertedTotal = await convertAmount(totalExpenses, req.user.company.currency, targetCurrency);
 ...
 const spendByCategory = async (req, res) => {
  try {
    ...
    const converted = await Promise.all(
      result.map(async (r) => ({
        category: r.category,
        totalAmount: await convertAmount(r._sum.amountCompany || 0, req.user.company.currency, targetCurrency),
      }))
    );
    ...
    console.error(err);
    res.status(500).json({ error: 'Failed to generate spend by category report' });
  }
};
...
const spendByUser = async (req, res) => {
  try {
    ...
    const withUserDetails = await Promise.all(
      result.map(async (r) => {
        const user = await prisma.user.findUnique({ where: { id: r.createdById } });
        const convertedAmount = await convertAmount(r._sum.amountCompany || 0, req.user.company.currency, targetCurrency);
        return { user: { id: user.id, name: user.name, email: user.email }, totalAmount: convertedAmount };
      })
    );
 ...
 const monthlyReport = async (req, res) => {
  try {
    ...
    for (const e of expenses) {
      ...
      const convertedAmount = await convertAmount(e.amountCompany || 0, req.user.company.currency, targetCurrency);
