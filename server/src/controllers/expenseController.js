...
const validateExpenseFields = (data) => {
  ...
  if (!data.amountOriginal || typeof data.amountOriginal !== 'number') errors.push('Amount is required and must be a number');
  if (!data.currencyOriginal || typeof data.currencyOriginal !== 'string') errors.push('Currency is required and must be a string');
  if (!data.category || typeof data.category !== 'string') errors.push('Category is required and must be a string');
  if (data.dateOfExpense && isNaN(new Date(data.dateOfExpense))) errors.push('Date of expense must be a valid date');
  return errors;
};
...
const getExpenseById = async (req, res) => {
  try {
    ...
    if (isNaN(id)) return res.status(400).json({ success: false, error: 'Invalid expense ID' });
...
const createExpense = async (req, res) => {
  try {
    ...
    if (currencyOriginal !== companyCurrency) {
      ...
      if (!exchangeRate) return res.status(500).json({ success: false, error: 'Failed to fetch exchange rate' });
...
const updateExpense = async (req, res) => {
  try {
    ...
    if (existing.createdById !== req.user.id) return res.status(403).json({ success: false, error: 'Not allowed to edit this expense' });
    ...
    if ((amountOriginal && currencyOriginal) &&
        (amountOriginal !== existing.amountOriginal || currencyOriginal !== existing.currencyOriginal)) {
      const company = await prisma.company.findUnique({ where: { id: req.user.companyId } });
      const companyCurrency = company?.currency || 'INR';
      exchangeRate = await getExchangeRate(currencyOriginal, companyCurrency);
      if (!exchangeRate) return res.status(500).json({ success: false, error: 'Failed to fetch exchange rate' });
    ...
    const updated = await prisma.expense.update({
      ...
      data: {
        ...
        description,
        category,
        dateOfExpense: dateOfExpense ? new Date(dateOfExpense) : existing.dateOfExpense,
...
const submitExpense = async (req, res) => {
  try {
    ...
    const expense = await prisma.expense.updateMany({
      where: { id, companyId: req.user.companyId, createdById: req.user.id, status: 'DRAFT' },
      data: { status: 'SUBMITTED', submittedAt: new Date() },
    });

    if (expense.count === 0) return res.status(404).json({ success: false, error: 'Expense not found or cannot be submitted' });
    return res.json({ success: true, message: 'Expense submitted successfully' });
  } catch (err) {
    console.error(err);
...
const deleteExpense = async (req, res) => {
  try {
    ...
    if (expense.count === 0) return res.status(404).json({ success: false, error: 'Expense not found or cannot be deleted' });
...
const getUserExpenses = async (req, res) => {
  try {
    ...
    console.error(err);
    return res.status(500).json({ success: false, error: 'Failed to fetch user expenses' });
  }
};
